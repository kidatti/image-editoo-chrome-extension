...
### 切り抜き
1. 左サイドバーの「切り抜き」ツールを選択
2. 表示されるピクセル寸法を確認しながら、残したい範囲をドラッグして指定
3. 選択範囲の内側をドラッグして位置を調整
4. 「切り抜く」またはEnterキーで確定（Escapeキーでキャンセル）

### ズーム・パン
- 右下に現在の画像サイズ（幅 × 高さ px）を常時表示
- 右下の `−` / `＋` またはマウスホイールで拡大縮小
- 手のひらボタン、またはSpaceキーを押しながらドラッグして移動
- 倍率表示をクリック、またはCtrl/Cmd+0で画面に合わせる
